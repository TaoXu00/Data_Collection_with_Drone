...
class Exp6_CPS_Solar_Dataset:
    ...
    def exp_6_my_solution(self,dir_mysolu, maximum_drone_energy_capacity, step_size,  dataset, sensor_map,
                           hovering_energy_per_unit, flying_energy_per_unit, number_of_training_data,
                           num_of_estimation_data, exp6_my_solu_plotter,size_data_collection, drone_commu_rate,
        ...
        with open('%spath_plan_mysolution.txt' %(dir_mysolu), 'w') as f:
            ...
            for drone_capacity in drone_capacity_list:
                ...
                if len(tour) != 0:
                    exp6_my_solu_plotter.plot_tour_map(sensor_map, tour, tour_id, optimal_dis, optimal_cost)
                    tour_id += 1
            averaged_mse_varying_drone_capabilities = np.average(mse_along_time_total, axis=1)
            exp6_my_solu_plotter.plot_averaged_mse_vary_drone_capability(drone_capacity_list,
                                                                 averaged_mse_varying_drone_capabilities,
                                                                 expected_mse_list)
            exp6_my_solu_plotter.plot_selected_sensors_vary_drone_capability(drone_capacity_list, num_of_selected_nodes)
            exp6_my_solu_plotter.plot_tour_length_vary_drone_capability(drone_capacity_list, optimal_dis_list)
            exp6_my_solu_plotter.plot_energy_cost_vary_drone_capability(drone_capacity_list, optimal_energy_cost_list)
    ...
    def exp6_ML_baseline(self, dir_baseline_ml, maximum_drone_energy_capacity, step_size,  dataset, sensor_map, hovering_energy_per_unit, flying_energy_per_unit, number_of_training_data, num_of_estimation_data, exp6_baseline_plotter, size_data_collection, drone_commu_rate,
                          mse_file_name, sensor_length_file_name, uav_speed, uav_unit_time_uav_operation_cost):
        mse_list_total = []
        num_of_selected_nodes = []
        optimal_dis_list = []
        ...
        with open('%spath_plan_baseline_ml.txt' % (dir_baseline_ml), 'w') as f:
            ...
            for drone_capacity in drone_capacity_list:
                ...
                if len(optimal_tour) != 0:
                    exp6_baseline_plotter.plot_tour_map(sensor_map, optimal_tour, tour_id, optimal_distance, optimal_cost)
                tour_id += 1
        # exp5_baseline_plotter.plotter.plot_mse_with_varying_drone_cap_for_different_ML_models(total_dist_list,
        #                                                              total_mse_list_for_all_ML_models_varying_drone_capability)
                ...
                if len(optimal_tour) != 0:
                    exp6_baseline_plotter.plot_tour_map(sensor_map, optimal_tour, tour_id, optimal_distance, optimal_cost)
                    tour_id += 1
            exp6_baseline_plotter.plot_mse_with_varying_drone_capabilities(drone_capacity_list, mse_list_total)
            exp6_baseline_plotter.plot_selected_sensors_vary_drone_capability(drone_capacity_list, num_of_selected_nodes)
            exp6_baseline_plotter.plot_tour_length_vary_drone_capability(drone_capacity_list, optimal_dis_list)
            exp6_baseline_plotter.plot_energy_cost_vary_drone_capability(drone_capacity_list, optimal_energy_cost_list)
    ...
    def exp6_FS_baseline(self,dir_baseline_fs, maximum_drone_energy_capacity, step_size, dataset, sensor_map,
                              hovering_energy_per_unit, flying_energy_per_unit, number_of_training_data,
                              num_of_estimation_data, exp6_baseline_fs_plotter, size_data_collection, drone_commu_rate,
                              mse_file_name, sensor_length_file_name, uav_speed, uav_unit_time_uav_operation_cost):
        ...
        with open('%spath_plan_baseline_fs.txt' % (dir_baseline_fs), 'w') as f:
            ...
            for drone_capacity in drone_capacity_list:
                ...
                if len(optimal_tour) != 0:
                    exp6_baseline_fs_plotter.plot_tour_map(sensor_map, optimal_tour, tour_id, optimal_distance,
                                                        optimal_cost)
                tour_id += 1
                # exp6_baseline_plotter.plotter.plot_mse_with_varying_drone_cap_for_different_ML_models(total_dist_list,
                #                                                              total_mse_list_for_all_ML_models_varying_drone_capability)
                ...
                if len(optimal_tour) != 0:
                    exp6_baseline_fs_plotter.plot_tour_map(sensor_map, optimal_tour, tour_id, optimal_distance,
                                                        optimal_cost)
                    tour_id += 1
            exp6_baseline_fs_plotter.plot_mse_with_varying_drone_capabilities(drone_capacity_list, mse_list_total)
            exp6_baseline_fs_plotter.plot_selected_sensors_vary_drone_capability(drone_capacity_list, num_of_selected_nodes)
            exp6_baseline_fs_plotter.plot_tour_length_vary_drone_capability(drone_capacity_list, optimal_dis_list)
            exp6_baseline_fs_plotter.plot_energy_cost_vary_drone_capability(drone_capacity_list, optimal_energy_cost_list)
    ...
    def exp_6(self, dir_exp6):
        #todo run_exp5 and check the results
        # This set of experiments varying the drone capability
        ## read the parameters of exp5
        path_sensor_map = self.config['Exp_6']['dir_sensor_map']
        dir_dataset = self.config['Exp_6']['dir_dataset']
        dir_mysolu= self.config['Exp_6']['dir_mysolu']
        dir_baseline_ml = self.config['Exp_6']['dir_baseline_ml']
        dir_baseline_fs = self.config['Exp_6']['dir_baseline_fs']
        maximum_drone_energy_capacity =int(self.config['Exp_6']['maximum_energy_capacity'])
        step_size = int(self.config['Exp_6']['step_size'])
        ...
        number_of_training_data= int(self.config['Exp_6']['num_of_training_data'])
        num_of_estimation_data= int(self.config['Exp_6']['num_of_estimation_data'])
        size_data_collection = int(self.config['Exp_6']['size_data_collection'])
        drone_commu_rate = int(self.config['Drone']['comm_rate'])
        mse_file_name = self.config['Exp_6']['mse_file_name']
        sensor_length_file_name=self.config['Exp_6']['sensor_length_file_name']
        ...
        self.plotter = plotter.plotter(dir_exp6)
        ...
        #
        # self.exp_6_my_solution(dir_mysolu+'/', maximum_drone_energy_capacity, step_size,  dataset, sensor_map, hovering_energy_per_unit, flying_energy_per_unit, number_of_training_data, num_of_estimation_data, exp6_my_solu_plotter,size_data_collection,
        ...
        # exp6_baseline_ml_plotter = plotter.plotter(dir_baseline_ml+'/')
        # self.exp6_ML_baseline(dir_baseline_ml+'/', maximum_drone_energy_capacity, step_size,  dataset, sensor_map,
        #                       hovering_energy_per_unit, flying_energy_per_unit, number_of_training_data, num_of_estimation_data,
        #                       exp6_baseline_ml_plotter, size_data_collection, drone_commu_rate, mse_file_name, sensor_length_file_name, uav_speed, uav_unit_time_uav_operation_cost)
        #
        # # #----------------------------Exp6 Baseline_fs ------------------------------------------------------------------
        #
        ...
        # exp6_baseline_fs_plotter = plotter.plotter(dir_baseline_fs+'/')
        # self.exp6_FS_baseline(dir_baseline_fs+'/', maximum_drone_energy_capacity, step_size, dataset, sensor_map,
        #                       hovering_energy_per_unit, flying_energy_per_unit, number_of_training_data,
        #                       num_of_estimation_data, exp6_baseline_fs_plotter, size_data_collection, drone_commu_rate,
        #                       mse_file_name, sensor_length_file_name, uav_speed, uav_unit_time_uav_operation_cost)
