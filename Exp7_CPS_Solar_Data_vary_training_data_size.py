...
class Exp7:
    ...
    def exp_7_my_solution(self,dir_mysolu, drone_energy_capacity, step_size,  dataset, sensor_map, hovering_energy_per_unit,
                          flying_energy_per_unit, maximum_num_of_training_data, num_of_estimation_data,
                          exp7_my_solu_plotter,size_data_collection, drone_commu_rate,mse_file_name, sensor_length_file_name, uav_speed, uav_unit_time_uav_operation_cost):
        expected_mse_list = []
        mse_along_time_total = []
        num_of_selected_nodes = []
        optimal_dis_list = []
        optimal_energy_cost_list = []
        num_of_training_dataset_size_list=range(600, maximum_num_of_training_data, step_size)
        with open('%spath_plan_mysolution.txt' %(dir_mysolu), 'w') as f:
            ...
            for num_of_training_data in num_of_training_dataset_size_list:
                ...
                if len(tour) != 0:
                    exp7_my_solu_plotter.plot_tour_map(sensor_map, tour, tour_id, optimal_dis, optimal_cost)
                    tour_id += 1
            averaged_mse_varying_training_dataset_size= np.average(mse_along_time_total, axis=1)
            exp7_my_solu_plotter.plot_averaged_mse_vary_training_dataset_size_with_expected_value(num_of_training_dataset_size_list,
                                                                 averaged_mse_varying_training_dataset_size,
                                                                 expected_mse_list)
            exp7_my_solu_plotter.plot_selected_sensors_varying_training_dataset_size(num_of_training_dataset_size_list, num_of_selected_nodes)
            exp7_my_solu_plotter.plot_tour_length_varying_training_dataset_size(num_of_training_dataset_size_list, optimal_dis_list)
            exp7_my_solu_plotter.plot_energy_cost_varying_training_dataset_size(num_of_training_dataset_size_list, optimal_energy_cost_list)
    ...
    def exp7_ML_baseline(self, dir_baseline_ml, drone_energy_capacity, step_size,  dataset, sensor_map,
                                  hovering_energy_per_unit, flying_energy_per_unit, maximum_num_of_training_data, num_of_estimation_data,
                                  exp7_baseline_ml_plotter, size_data_collection, drone_commu_rate, mse_file_name, sensor_length_file_name, uav_speed, uav_unit_time_uav_operation_cost):
        mse_list_total = []
        num_of_selected_nodes = []
        optimal_dis_list = []
        optimal_energy_cost_list = []
        training_dataset_size_list = range(600, maximum_num_of_training_data, step_size)
        with open('%spath_plan_baseline_ml.txt' % (dir_baseline_ml), 'w') as f:
            ...
            for training_dataset_size in training_dataset_size_list:
                ...
                if len(optimal_tour) != 0:
                    exp7_baseline_ml_plotter.plot_tour_map(sensor_map, optimal_tour, tour_id, optimal_distance, optimal_cost)
                tour_id += 1
            ...
            exp7_baseline_ml_plotter.plot_averaged_mse_vary_training_dataset_size(training_dataset_size_list,
                                                                              mse_list_total)
            exp7_baseline_ml_plotter.plot_selected_sensors_varying_training_dataset_size(training_dataset_size_list,
                                                                                     mse_list_total)
            exp7_baseline_ml_plotter.plot_tour_length_varying_training_dataset_size(training_dataset_size_list,
                                                                                mse_list_total)
            exp7_baseline_ml_plotter.plot_energy_cost_varying_training_dataset_size(training_dataset_size_list,
                                                                                mse_list_total)
    ...
    def exp7_FS_baseline(self,dir_baseline_fs,drone_energy_capacity, step_size, dataset, sensor_map,
                              hovering_energy_per_unit, flying_energy_per_unit, maximum_number_of_training_data,
                              num_of_estimation_data, exp7_baseline_fs_plotter, size_data_collection, drone_commu_rate,
        ...
        optimal_dis_list = []
        optimal_energy_cost_list = []
        training_dataset_size_list = range(600, maximum_number_of_training_data, step_size)
        with open('%spath_plan_baseline_fs.txt' % (dir_baseline_fs), 'w') as f:
            ...
            for training_dataseet_size in training_dataset_size_list:
                ...
                if len(optimal_tour) != 0:
                    exp7_baseline_fs_plotter.plot_tour_map(sensor_map, optimal_tour, tour_id, optimal_distance,
                                                        optimal_cost)
                tour_id += 1
                ...
                if len(optimal_tour) != 0:
                    exp7_baseline_fs_plotter.plot_tour_map(sensor_map, optimal_tour, tour_id, optimal_distance,
                                                        optimal_cost)
                    tour_id += 1
            exp7_baseline_fs_plotter.plot_mse_with_varying_drone_capabilities(training_dataset_size_list, mse_list_total)
            exp7_baseline_fs_plotter.plot_selected_sensors_vary_drone_capability(training_dataset_size_list, num_of_selected_nodes)
            exp7_baseline_fs_plotter.plot_tour_length_vary_drone_capability(training_dataset_size_list, optimal_dis_list)
            exp7_baseline_fs_plotter.plot_energy_cost_vary_drone_capability(training_dataset_size_list, optimal_energy_cost_list)
    ...
    def exp_7(self, dir_exp7):
        # This set of experiments varying the training data size
        dir_sensor_map = self.config['Exp_7']['dir_sensor_map']
        dir_dataset = self.config['Exp_7']['dir_dataset']
        dir_mysolu = self.config['Exp_7']['dir_mysolu']
        dir_baseline_ml = self.config['Exp_7']['dir_baseline_ml']
        dir_baseline_fs = self.config['Exp_7']['dir_baseline_fs']

        # dir_real_mean_of_sensors = self.config['Exp_7']['dir_real_mean_of_sensors']
        drone_energy_capacity= int(self.config['Exp_7']['drone_energy_capacity'])
        step_size = int(self.config['Exp_7']['step_size'])
        ...
        maximum_num_of_training_data = int(self.config['Exp_7']['maximum_num_of_training_data'])
        num_of_estimation_data = int(self.config['Exp_7']['num_of_estimation_data'])
        size_data_collection = int(self.config['Exp_7']['size_data_collection'])
        drone_commu_rate = int(self.config['Drone']['comm_rate'])
        mse_file_name = self.config['Exp_7']['mse_file_name']
        sensor_length_file_name = self.config['Exp_7']['sensor_length_file_name']
        ...
        self.plotter = plotter.plotter(dir_exp7)
        ...
        sensor_map = json.load(open(dir_sensor_map))

        # load dataset
        #name, ext = os.path.splitext(dir_sensor_map)
        ...
        # exp2_my_solu_plotter = plotter.plotter(dir_mysolu+'/')
        #
        # self.exp_7_my_solution(dir_mysolu+'/', drone_energy_capacity, step_size,  dataset, sensor_map, hovering_energy_per_unit, flying_energy_per_unit, maximum_num_of_training_data, num_of_estimation_data, exp2_my_solu_plotter,size_data_collection,
        ...
        # exp2_baseline_ml_plotter = plotter.plotter(dir_baseline_ml+'/')
        # self.exp7_ML_baseline(dir_baseline_ml+'/', drone_energy_capacity, step_size,  dataset, sensor_map,
        #                       hovering_energy_per_unit, flying_energy_per_unit, maximum_num_of_training_data, num_of_estimation_data,
        #                       exp2_baseline_ml_plotter, size_data_collection, drone_commu_rate, mse_file_name, sensor_length_file_name, uav_speed, uav_unit_time_uav_operation_cost)
        #
        # # # ----------------------------Exp7 Baseline_fs ------------------------------------------------------------------
        # if not os.path.exists(dir_baseline_fs):
        #     os.makedirs(dir_baseline_fs)
        #     os.chmod(dir_baseline_fs, 0o700)
        # exp1_baseline_fs_plotter = plotter.plotter(dir_baseline_fs+'/')
        # self.exp7_FS_baseline(dir_baseline_fs+'/', drone_energy_capacity, step_size, dataset, sensor_map,
        ...
        mse_metric = self.config['Exp_7']['mse_matric_name']
        selected_sensor_metric = self.config['Exp_7']['selected_sensor_matric_name']
        # make the final plot of the metric with three solutions
        matrics = [mse_metric, selected_sensor_metric]
        training_dataset_size_list = np.arange(600, maximum_num_of_training_data, step_size)
        for metric in matrics:
            ...
            for dir_solu in dirs_solu:
                ...
                if metric == mse_metric:
                    data = np.loadtxt("%s%s" % (dir_solu, mse_file_name))
                elif metric == selected_sensor_metric:
                    data = np.loadtxt("%s%s" % (dir_solu, sensor_length_file_name))
                avgs[solu_name] = data
            if metric == mse_metric:
                self.plotter.plot_MSE_with_all_solutions_exp7(training_dataset_size_list, avgs)
            else:
